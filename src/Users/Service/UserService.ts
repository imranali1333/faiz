...
export interface AuthData {
  ...
  photo_url?: string;
  auth_date: string;
  hash: string; // Not used in the upsert but included for completeness
  phone_number?: string; // Made optional
  telegramId: string;
  language_code?: string; // Added to match schema
  is_bot?: boolean; // Added to match schema
}
...
export async function userAuthentication(authData: AuthData): Promise<void> {
  try {
    ...
    const {
      ...
      photo_url = '',
      auth_date,
      phone_number = '', // Default to empty if not provided
      telegramId,
      hash,
      language_code = '', // Default to empty if not provided
